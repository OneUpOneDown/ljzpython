...
def get_screen_image():
	os.system('adb shell screencap -p /sdcard/screen.png')#获取当前界面手机截图
...
def update_screen(frame):#更新图片、重画图片
	global need_update 
	if need_update:
		time.sleep(1)
...
axes_image = plt.imshow(get_screen_image(),animated=True)#把回去的图片画在坐标轴上面
